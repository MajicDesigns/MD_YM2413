...
Custom instruments may be defined.

Various settings are available, defined by the hardware. 

Amplitude and Frequency modulation.

These are summarised in the table below and are reflected in the data structure
used to set the instrument parameters.

|Parameter            | D/sheet | Cmd | Mod     | Mod Reg | Carrier | Car Reg |
|:--------------------|:--------|:---:|:--------|:--------|:--------|:--------|
|Amplitude Modulation | AM      |AM   | Boolean | 00.7    | Boolean | 01.7    |
|Vibrato              | VIB     | V   | Boolean | 00.6    | Boolean | 02.6    |
|Envelope Generation  | EGTYP   | E   | 0..1    | 00.5    | 0..1    | 03.5    |
|Key Scale Rate       | KSR     | KR  | Boolean | 00.4    | Boolean | 04.4    |
|Wave Selection       | MULTI   | W   | 0..7    | 00.3-0  | 0..7    | 05.3-0  |
|Key Scale Level      | KSL     | KL  | 0..3    | 02.7-6  | 0..3    | 03.7-6  |
|Modulation Level     | TL      | ML  | 0..63   | 02.5-0  | N/A     | N/A     |
|FM Feedback          | FB      | FF  | N/A     | N/A     | 0..7    | 03.2-0  |
|Rectify              | DM, DC  | R   | Boolean | 03.03   | Boolean | 03.04   |
|Attack               | AR      | A   | 0..15   | 04.7-4  | 0..15   | 05.7-4  |
|Decay                | DR      | D   | 0..15   | 04.3-0  | 0..15   | 05.3-0  |
|Sustain              | SL      | S   | 0..15   | 06.7-4  | 0..15   | 07.7-4  |
|Release              | RR      | R   | 0..15   | 06.3-0  | 0..15   | 07.3-0  |
...
class MD_YM2413
{
  public:
    ...
    static const uint8_t ENV_PARAM = 2;     ///< modulator and carrier parameters
    static const uint8_t MPARAM = 0;        ///< index for modulator parameter
    static const uint8_t CPARAM = 1;        ///< index for carrier parameter
    ...
    */
    ...
    /**
     * Data structure used to define envelope for custom instrument types.
     * See \ref pageCustom for more information. This definition is for 
     * convenience and is repackaged into the correct registers for the device.
     */
    struct envelope_t
    {
      bool pAM;       ///< Amplitude Modulation
      bool pVIB;      ///< Vibrato
      bool pEGTYP;    ///< Envelope Generation
      bool pKSR;      ///< Key Scale Rate
      uint8_t pMULTI; ///< Wave Selection
      uint8_t pKSL;   ///< Key Scale Level
      uint16_t pTL;   ///< Modulation Level (valid for Modulator only)
      uint8_t pFB;    ///< FM Feedback (valid for Modulator only)
      bool pDMC;      ///< Rectify (DM for Modulator, DC for Carrier)
      uint8_t pAR;    ///< Attack
      uint8_t pDR;    ///< Decay
      uint8_t pSL;    ///< Sustain
      uint8_t pRR;    ///< Release
    ...
    */
     ...
     *
     * Define the playing envelope for the custome instrument so that it can 
     * be assigned to a channel. The parameters are passed in the envelope_t
     * structure.
     *
     * ENV_PARAM structures need to be defined as an array. The Modulator definition 
     * with the structure is at array reference MPARAM and the Carrier definition
     * at the array reference CPARAM.
     *
     * \sa envelope_t, \ref pageCustom
     *
     * \param env     refernce to an array of ENV_PARAM envelope_t structures.
     */
    void defineInstrument(const envelope_t *env);
     ...
     * for the device and will be passed through without further processing.
     *
     * This method is used to set instrument defineions that are predefined 
     * and held in the application as a compact byte sequence.
     *
     * \sa \ref pageCustom
     *
     * \param data    an array of 8 bytes that will be written to registers 0x00 through 0x07.
     */
    void defineInstrument(const uint8_t *data) { for (uint8_t i = 0; i < 8; i++) send(i, data[i]); }
